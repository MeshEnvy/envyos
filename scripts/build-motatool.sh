...
usage: $0 [--host-only] [--no-docker] [--target <platform>]…
  ...
  Build motatool release binaries staged as motatool-<platform> under
  build/motatool/<motatool>/.
...
stage_built_platform() {
  ...
  out="$(motatool_staged_binary_path "$mt_ver" "$platform")"
  echo "    staged: $out"
}
...
build_motatool_platform_native() {
  ...
  if [[ "$platform" == "$host" ]]; then
    ...
    (cd "$ROOT/motatool" && PATH="$cargo_dir:$PATH" "$cargo_bin" build --release --target "$triple")
  fi
